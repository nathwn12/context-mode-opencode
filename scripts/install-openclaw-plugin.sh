#!/usr/bin/env bash
# One-shot installer: register context-mode-opencode plugin into a local OpenClaw instance.
...
EXT_DIR="$OPENCLAW_STATE_DIR/extensions/context-mode-opencode"

echo "→ context-mode-opencode plugin installer"
...
// context-mode-opencode OpenClaw global extension — auto-generated by install script.
...
echo "→ clearing jiti cache for context-mode-opencode..."
rm -f /tmp/jiti/context-mode-opencode-index.*.cjs /tmp/jiti/build-openclaw-plugin.*.cjs 2>/dev/null || true
...
if [ -n "$OPENCLAW_BIN" ] && OPENCLAW_STATE_DIR="$OPENCLAW_STATE_DIR" "$OPENCLAW_BIN" plugins list 2>&1 | grep -q 'context-mode-opencode'; then
  echo "  ✓ context-mode-opencode discovered"
else
...
if (!allow.includes('context-mode-opencode')) allow.unshift('context-mode-opencode');

// Add to plugins.entries
const entries = plugins.entries ??= {};
if (!entries['context-mode-opencode']) entries['context-mode-opencode'] = { enabled: true };
...
echo ""
echo "✓ done — context-mode-opencode plugin installed and active"
