#!/usr/bin/env bash
# context-mode-opencode diagnostic report
...
# ─── 3. context-mode-opencode Installation ────────────────────────────────────────────

section "3. context-mode-opencode Installation"
...
# npm latest
NPM_LATEST="$(safe_cmd_quiet npm view context-mode-opencode version 2>/dev/null)"
...
# Detection logic (mirrors context-mode-opencode adapter selection)
...
if [ -d "$HOME_DIR/.openclaw/extensions/context-mode-opencode" ]; then
  printf -- '- **OpenClaw extension dir**: exists (`~/.openclaw/extensions/context-mode-opencode/`)\n'
else
...
FTS5_RESULT="$(safe_cmd_quiet node -e "
  try {
    ...
    db.exec(\"INSERT INTO t VALUES('context-mode-opencode diagnostic test')\");
...
if command -v ps &>/dev/null; then
  CTX_PROCS="$(ps aux 2>/dev/null | grep '[c]ontext-mode' | grep -v -E 'context-mode-opencode-dashboard|esbuild|wrangler|workerd|grep' || true)"
...
kv "Running context-mode-opencode processes" "$CTX_COUNT"
...
if [ "$CTX_COUNT" -gt 3 ]; then
  warn "Multiple context-mode-opencode processes — possible orphans"
fi
...
SESSION_DIRS=(
  "$HOME_DIR/.claude/context-mode-opencode/sessions"
  "$HOME_DIR/.cursor/context-mode-opencode/sessions"
  "$HOME_DIR/.codex/context-mode-opencode/sessions"
  "$HOME_DIR/.gemini/context-mode-opencode/sessions"
  "$HOME_DIR/.openclaw/context-mode-opencode/sessions"
  "$HOME_DIR/.kiro/context-mode-opencode/sessions"
)
TOTAL_DB_COUNT=0
...
# context-mode-opencode specific
...
SESSION_BASE="$HOME_DIR/.claude/context-mode-opencode/sessions"
...
NPM_TLS="$(timed 10 node -e "
  ...
  const req = https.get('https://registry.npmjs.org/@nathwn12%2Fcontext-mode-opencode', { timeout: 5000 }, (res) => {
...
printf '\n'
printf '  context-mode-opencode diagnostic v%s\n' "$CTX_DEBUG_VERSION"
printf '  ─────────────────────────────\n'
