...
description: |
  Use context-mode-opencode tools (ctx_execute, ctx_execute_file) instead of Bash/cat when processing
...
<context_mode_logic>
  <mandatory_rule>
    Default to context-mode-opencode for ALL commands. Only use Bash for guaranteed-small-output operations.
  </mandatory_rule>
</context_mode_logic>
...
**When uncertain, use context-mode-opencode.** Every KB of unnecessary context reduces the quality and speed of the entire session.
...
│   └── Use context-mode-opencode ctx_execute or ctx_execute_file
│
...
Use context-mode-opencode for ANY of these, without being asked:
...
4. **For files you need to EDIT**: Use the normal Read tool. context-mode-opencode is for analysis, not editing.
5. **For Bash whitelist commands only**: Use Bash for file mutations, git writes, navigation, process control, package install, and echo. Everything else goes through context-mode-opencode.
...
<sandboxed_data_workflow>
  ...
  <workflow>
    LargeDataTool(filename: "path") → mcp__context-mode-opencode__ctx_index(path: "path") → ctx_search()
  </workflow>
</sandboxed_data_workflow>
...
Subagents automatically receive context-mode-opencode tool routing via a PreToolUse hook. You do NOT need to manually add tool names to subagent prompts — the hook injects them. Just write natural task descriptions.
